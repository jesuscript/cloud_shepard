...
//TODO: merge configs and cli args in the beginning
...
module.exports = function(){
  ...
  ({
    "create": function(){
      ...
      async.each(config.droplets.hosts, function(host, cb){
        ...
        listDroplets(function(err, res, body){
          var hostsTotal = body.length;
          
          waitOperationEnd(function(body){
            var numDone = _.filter(body, {status: "active"}).length;
            ...
            getInventory(function(inventory){
              ...
              if(inventory) writeToFile(options.create["write-inventory-to"] ||
    ...
    "destroy": function(){
      listDroplets(function(err, res, body){
        async.each(_.map(body, "id"), function(id, cb){
          ...
          waitOperationEnd(function(body){
            report("Destroying " + total + " droplets", body.length, total);
    ...
    "inventory": function(){
      getInventory(function(inventory){
        if(inventory){
          ...
          writeToFile(options.inventory["write-inventory-to"] || config.ansibleInventory,
                      inventory);
        }else{
  ...
  function getInventory(cb){
    listDroplets(function(err, res, body){
      if(body.length){
        var i = 0,
            ips = _.map(body,"networks.v4.0.ip_address");
  ...
  function listDroplets(cb){
    api.dropletsGetAll({
      ...
      body = _.filter(body, {name: config.droplets.name});
      cb(err, res, body);
    });
  }
